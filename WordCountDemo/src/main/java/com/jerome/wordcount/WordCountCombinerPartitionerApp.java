...
import com.jerome.wordcount.component.CustomPartitioner;
...
public class WordCountCombinerPartitionerApp {
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException, URISyntaxException {
        ...
        // 8、设置 Mapper 阶段输出数据的 key 和 value 类型
        job.setMapOutputKeyClass(Text.class);
        job.setMapOutputValueClass(IntWritable.class);

        // 9、设置 Reducer 阶段输出数据的 key 和 value 类型
        ...
        // 10、如果输出目录已经存在，则必须先删除，否则重复运行程序时会抛出异常
        ...
        // 12、提交 job 到群集并等待它完成，参数设置为 true 代表打印显示对应的进度
